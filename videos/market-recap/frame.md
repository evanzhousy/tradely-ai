...
description: >
  ...
  are adapted for this market-recap edition — a pure white canvas, electric cobalt structural
  ink, black subtitles, a controlled chart-gradient palette, the permanent graph-paper grid,
  Newsreader serif 400 + Hanken Grotesk + DM Mono, cobalt hairlines, and the pixel-glitch +
  QR-block signatures. Composition, frame scale, and aspect-ratio behavior are rewritten for
  the frame. Motion is out of scope.
...
colors:
  ...
  chart-gex: ["#4C1D95", "#7B2CBF", "#B79CED"]
  chart-premium: ["#0B4FDB", "#1677E8", "#69B7FF"]
  chart-activity: ["#C2410C", "#F05A47", "#FDBA74"]
  chart-call-share: ["#047857", "#00B894", "#99F6E4"]
  chart-directional: ["#4338CA", "#6366F1", "#A5B4FC"]
...
components:
  ...
  pixel-stack-bar:
    cells: "column-reverse, {colors.grid} off / chart-gradient on"
    baseline: "0.12cqw solid {colors.ink} + {typography.mono-tick} ticks"
    description: "Data as grid-unit cells; every chart uses one assigned hue family with lightness-only gradient steps."
  lightweight-candlestick:
    engine: "TradingView Lightweight Charts 5.2, vendored locally"
    palette: "{colors.chart-directional}"
    behavior: "independent autoscale, five-session logical range, native price/time axes, dashed close line, TradingView attribution"
    description: "Professional OHLC primitive for SPX, TSLA, and NVDA. No external iframe or render-time network request."
  shadcn-chart-grammar:
    source: "shadcn/ui Chart catalog, Recharts v3 patterns"
    runtime: "deterministic local SVG/CSS adaptation for HyperFrames; no render-time React or network dependency"
    patterns: ["negative/grouped bar", "horizontal labeled bar", "donut / pie part-to-whole", "gradient area + line + square data points"]
    behavior: "explicit scales, persistent labels, no hover-only meaning, chart-family CSS color tokens"
    description: "Chart selection follows the financial meaning: signed exposure uses a zero axis, rankings use horizontal bars, a reconciled whole uses one donut, and prior-to-current movement uses line/area with a benchmark reference."
  vstack-label:
...
**Project-approved color system:** the market-recap uses pure white
`{colors.paper}` throughout, black `{colors.caption}` subtitle type, and a
controlled gradient palette only inside charts and data marks. Each chart uses
one hue family, varied only by lightness; typography never uses a gradient.
TradingFlow and referenced market-entity logos preserve their official colors. These rules
override the base preset's later cream/two-color/no-gradient language.
Editorial typography, rules, grid lines, and decoration remain cobalt.

Cobalt Grid at frame scale becomes a **white editorial market-data report** —
pure white canvas, electric cobalt structure, and a permanent graph-paper grid
behind every frame. Cobalt remains the editorial ink for headlines, body,
rules, grid, pixel-glitch decoration, and QR patches. Color appears only where
it carries data or preserves an official logo.
...
- **White editorial canvas** — pure white ground with cobalt typography, rules, grid, and chrome.
- **Chart-only color** — each chart uses one monochromatic gradient family; typography is always solid cobalt or black.
- **Black subtitles** — the dedicated lower rail uses high-contrast black type.
...
- **Pixel-stack bars** render data as grid-unit cells (chart-gradient on / 10% cobalt off).
- **Professional candlesticks** use the locally vendored TradingView Lightweight Charts primitive with native axes, close labels, and attribution.
- **shadcn/Recharts chart grammar** supplies negative/grouped bars, horizontal labeled rankings, one reconciled donut composition, and gradient line/area comparisons, adapted to deterministic SVG/CSS for video.
...
- **Restraint** — cobalt carries the editorial system; gradients are confined to charts and stay within one hue family per chart; one pixel-glitch column and at most one QR-block per declarative frame.
- **Reference** — aim at a **WIRED Japan / Shift data monograph on a crisp white sheet**; failure looks like a generic rainbow dashboard with color leaking into labels and chrome.
...
`{colors.paper}` is the pure-white ground; `{colors.ink}` cobalt is the
editorial ink for type, rules, grid lines, glitch, and QR.
`{colors.caption}` is reserved for black subtitle type. `{colors.grid}` (10%)
is the permanent grid and chart "off" cells; `{colors.ink-faint}` (18%) is for
row dividers. Chart marks use one assigned family per chart:
`{colors.chart-gex}`, `{colors.chart-premium}`, `{colors.chart-activity}`,
`{colors.chart-call-share}`, or `{colors.chart-directional}`. Stops within a
chart vary lightness, not hue. No text or other authored element uses a
gradient.
...
**Ceiling:** no drop shadow (the QR's 1.5px paper outset is an anti-shadow for readability, not elevation), no rounded corner, and no gradients outside charts/data marks.

## Shapes

- **0 radius everywhere** — frames, ledger rows, QR cells, glitch blocks, and chart containers. The sole circular exception is one data-bearing donut ring for a fully reconciled part-to-whole composition; no decorative circles.
...
**Ground** white paper + grid, hairlines, `pad-top`. **Composes** topbar-rule, pixel-stack-bar row. **Focal** a row of 6–8 pixel-stack bars (chart-gradient on / 10% cobalt off) over a 1.5px cobalt baseline with mono ticks. **Chrome** topbar headline + mono fig-tag; page number. **Accent** chart-only gradient. **Silence** moderate. **Fixed** grid-unit cells, cobalt baseline, mono ticks. **Free** bar values (from script), tick labels, gradient direction by series. **Density** standard/dense.
...
- Render data as **pixel-stack cells** (single-family chart-gradient on / 10% cobalt off); echo the glitch language.
...
- Don't let chart colors leak into editorial typography, rules, grid, or page chrome.
- Don't combine unrelated hues within one chart gradient.
- Don't use pie/donut charts for rankings, signed values, or trends; the only allowed donut must reconcile a documented whole to 100%.
...
- **Color discipline** — white canvas, cobalt editorial system, black subtitle type, gradients only inside data marks, one hue family per chart.
