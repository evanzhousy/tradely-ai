...
Current baseline: Lessons 1–28 have implemented SVG labs and individual review records. Final curriculum-wide completion still needs a fresh audit.
...
- Next implementation: Lesson 29, `rank-contracts`. Lesson 28 evidence and checks are recorded in `lesson-28-svg-lab-2026-09-12.md`.
...
- Lesson 29 (`rank-contracts`): inspect strike/expiry neighborhoods under a fixed underlying, option type, session and quality rule. Compare total, peak and breadth without turning missing or prior-session values into eligible zeros. Expiry visibility does not change scope; moneyness is context, not a buy recommendation. Preserve the existing investigation-stage neighborhood controls. This is paid: verify the anonymous gate separately from the local component preview.
...
- The owned production preview on port 8252 was stopped after Lesson 28 verification, ready for the next rebuild.
