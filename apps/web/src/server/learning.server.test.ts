...
describe("learning persistence and authorization (isolated PostgreSQL)", () => {
	...
	async function finish(id = lessonId) {
		...
		const scenario = required(
			getLessonScenarios(id).find((item) => item.id === view.scenarioId),
		);
		for (const step of scenario.steps) {
			for (const question of step.questions)
				view = viewOf(
					await updateLearningImpl({
						...command(view, referenceAction(question)),
						lessonId: id,
					}),
				);
			view = viewOf(
				await updateLearningImpl({
					...command(view, { type: "submit" }),
					lessonId: id,
				}),
			);
	...
	it("atomically resolves concurrent numerical decisions without overwriting the winner", async () => {
		...
		const results = await Promise.all(
			["63000", "64000"].map((value) =>
				updateLearningImpl(
					command(first, { type: "respond", questionId: "premium", value }),
				),
			),
		);
	...
	it("rejects a cached tab update to a known earlier active case without changing its record", async () => {
		...
		await db
			...
			.where(eq(schema.lessonAttempt.id, initial.attemptId));
		expect(
			await updateLearningImpl(command(initial, { type: "hint" })),
		).toEqual({ ok: false, reason: "retired" });
		const [preserved] = await db.select().from(schema.lessonAttempt);
		expect(preserved).toMatchObject({
			status: "in_progress",
			revision: initial.revision,
			state: initialAttemptState(),
		});
		expect(viewOf(await open(true)).scenarioId).toBe(
			"validate-option-print-practice-1",
		);
		const [old] = await db
			.select()
	...
	it("preserves an archived submitted scenario without certifying the updated lesson", async () => {
		...
		for (const step of legacy.steps) {
			for (const evidenceId of step.requiredEvidence)
				state = transitionAttempt(legacy, state, {
					type: "inspect",
					evidenceId,
				});
			for (const question of step.questions)
				state = transitionAttempt(legacy, state, referenceAction(question));
			state = transitionAttempt(legacy, state, { type: "submit" });
			if (state.phase !== "complete")
				state = transitionAttempt(legacy, state, { type: "continue" });
		}
		await db.insert(schema.lessonAttempt).values({
			...
			status: "submitted",
			state,
			assessment: assessAttempt(legacy, state),
			submittedAt: new Date(),
		});
		const old = viewOf(await open());
		expect(old.archived).toBe(true);
		expect(old.result?.met).toBe(3);
		expect(viewOf(await open(true)).scenarioId).toBe(
			"validate-option-print-practice-1",
		);
