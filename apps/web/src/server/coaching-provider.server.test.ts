...
vi.mock("@tradely/env/server", () => ({
	env: { AI_GATEWAY_API_KEY: "test-only-inert-key" },
}));
vi.mock("ai", () => ({
...
describe("coaching provider contract", () => {
	...
	beforeEach(() => {
		vi.clearAllMocks();
		mocks.gateway.mockReturnValue((model: string) => model);
		mocks.generate.mockResolvedValue({
			output: exampleFeedback(snapshot),
	...
	it("uses bounded structured output, one provider, zero retries and no text telemetry", async () => {
		...
		expect(options.prompt).not.toContain("learner-a");
		expect(options.prompt).not.toContain('"accepted"');
	});
	it("rejects unknown references or false blanket approval", async () => {
		mocks.generate.mockResolvedValue({
			output: {
				...exampleFeedback(snapshot),
				gaps: [
					{
						criterionId: "comparison",
						text: "hidden",
						referenceIds: ["future-case"],
					},
				],
			},
			usage: {},
		});
		await expect(generateCoachingFeedback(input)).rejects.toThrow(
			"invalid_output",
		);
		mocks.generate.mockResolvedValue({
			output: { ...exampleFeedback(snapshot), gaps: [], question: null },
			usage: {},
		});
		await expect(
			generateCoachingFeedback({
				...
				snapshot: {
					...snapshot,
					answers: [{ question: "ratio", answer: "900", checked: false }],
				},
			}),
