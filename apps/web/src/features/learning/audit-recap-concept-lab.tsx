...
const scenes = [
	{
		...
		steps: teachingSteps(
			...
			[
				["Read source values", "读取来源数值"],
				["Restore the multiplier", "恢复乘数"],
				["Verify repaired amount", "核对修复金额"],
			],
		),
	...
	},
	{
		...
		steps: teachingSteps(
			...
			[
				["Audit each claim", "审计每项结论"],
				["Repair one defect", "修复一个缺陷"],
				["Recheck the report", "重新检查报告"],
			],
		),
	...
	},
	{
		...
		steps: teachingSteps(
			...
			[
				["Review repaired evidence", "检查修复后证据"],
				["List open gaps", "列出未解决缺口"],
				["Write bounded signoff", "写出有边界的签核"],
			],
		),
