		...
		<SceneLayout
			comparison={
				<BeforeAfterComparison
					locale={locale}
					comparisonKey={`${id}:${method}:${fees}`}
					values={[
						{
							id: "realized",
							label: l("Realized P&L", "已实现盈亏"),
							before: 0,
							current: result.realized,
							unit: "USD",
							format: money,
						},
						{
							id: "unrealized",
							label: l("Unrealized P&L", "未实现盈亏"),
							before:
								stockAccounting(
									lots,
									{ ...close, quantity: 0 },
									knownMark ? source.initialMark : null,
									method,
									fees !== "exclude",
								)?.unrealized ?? null,
							current: result.unrealized,
							unit: "USD",
							format: money,
						},
					]}
					note={l(
						"Same lot and fee basis, from the supplied opening checkpoint.",
						"采用相同批次与费用口径，从给定起始检查点比较。",
					)}
				/>
			}
			diagram={
					...
					items={[
						{
							id: "result-1",
							label: <>{l("Realized P&L", "已实现盈亏")}</>,
							value: <>{money(result.realized, true)}</>,
						},
						{
							id: "result-2",
							label: <>{l("Unrealized P&L", "未实现盈亏")}</>,
							value: <>{money(result.unrealized, true)}</>,
						},
						{
							id: "result-3",
							label: <>{l("Remaining value", "剩余估值")}</>,
			...
			details={
				<>
					...
					<p data-pnl-total>
						{l("Same-lot total P&L", "同批次总盈亏")}:{" "}
						{money(result.total, true)}
					</p>
					<p className="font-mono text-xs">
						...
						{lots
							.map(
								(lot) =>
									`${lot.id} ${lot.openedAt} · fee ${money(lot.feeCents)}`,
							)
							.join("; ")}
						<br />
