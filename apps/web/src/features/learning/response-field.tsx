...
export function ResponseField({
	...
	useEffect(() => {
		onDraftChange(question.id, draft !== value);
		return () => onDraftChange(question.id, false);
	}, [draft, value, onDraftChange, question.id]);
	const input = question.input;
	if (!input) return null;
	const valid = responseComplete(question, draft);
	const dirty = draft !== value;
	return (
		<Field data-invalid={!!draft && !valid} data-disabled={disabled}>
			...
			{input.kind === "number" ? (
				<Input
					...
					onKeyDown={(event) => {
						if (event.key === "Enter" && valid && dirty) {
			...
			{dirty ? (
				<p className="text-xs" role="status">
					{locale === "zh"
						? "此修改尚未保存。继续前请保存。"
						: "This edit is not saved. Save it before continuing."}
				</p>
			) : null}
		</Field>
	);
