...
const view = projectAttempt(
	getLessonScenarios(record.lessonId)[0],
	attemptStateSchema.parse(record.state),
	record.id,
	record.revision,
);
const initial: CoachingResponse = {
	ok: true,
	view: {
		available: true,
		reason: null,
...
const props = () => ({
	...
	blocked: false,
	onHint: vi.fn(),
	transport: coachingFixture(() => record),
});

describe("inline coaching", () => {
	it("distinguishes coaching from guided submission and preserves an unfinished coaching draft", async () => {
		render(
			<LearningScreen
				view={view}
				locale="en"
				...
				onOpen={vi.fn()}
				onRecover={vi.fn()}
				coachingTransport={coachingFixture(() => record)}
			/>,
		);
		const field = await screen.findByRole("textbox", {
			name: "Explain your judgment",
		});
		expect(
			screen.getAllByRole("button", { name: "Review my reasoning" }),
		).toHaveLength(1);
	...
	it("saves, reviews, revises and deletes a complete cycle with private text masked", async () => {
		...
		fireEvent.click(
			screen.getByRole("button", { name: "Work through a contrasting case" }),
		);
		expect(screen.getByText(/Research question: which of two/)).toBeTruthy();
		fireEvent.change(field, {
			target: {
				value: `${exampleReason} I compare each symbol with its own baseline.`,
			},
		});
		fireEvent.click(screen.getByRole("button", { name: "Save explanation" }));
		await waitFor(() =>
		...
		expect(
			screen
				...
				.closest("[data-analytics-private]"),
		).toBeTruthy();
		fireEvent.click(
			screen.getByRole("button", { name: "Delete coaching record" }),
		);
	...
	it("reuses saved written answers and blocks calls for unsaved lesson edits", async () => {
		...
		const v = projectAttempt(
			getLessonScenarios(r.lessonId)[0],
			attemptStateSchema.parse(r.state),
			r.id,
			0,
		);
		render(
	...
	it("hides a disabled feature and does not offer generation on independent cases", async () => {
		...
		await act(async () => {});
		expect(screen.queryByText("Check my reasoning")).toBeNull();
		rerender(
			<CoachingPanel
				{...p}
				key="independent"
				view={{ ...view, step: { ...view.step, kind: "independent" } }}
			/>,
		);
		await act(async () => {});
		expect(
			screen.queryByRole("button", { name: "Review my reasoning" }),
		).toBeNull();
	});
	it("ignores a prior account's late reply after remount", async () => {
		let resolve!: (response: CoachingResponse) => void;
		const first: CoachingTransport = {
			read: () =>
				new Promise((r) => {
					resolve = r;
				}),
		...
		const { rerender } = render(
			<CoachingPanel {...props()} key="account-a" transport={first} />,
		);
		rerender(
			<CoachingPanel
				{...props()}
				key="account-b"
				transport={{
					read: async () => ({ ok: false, reason: "not_eligible" }),
					update: vi.fn(),
				}}
			/>,
		);
	...
	it("recovers a failed read without losing a typed explanation", async () => {
		...
		render(
			<CoachingPanel
				{...p}
				transport={{ read: async () => initial, update }}
			/>,
		);
		fireEvent.change(await screen.findByRole("textbox"), {
			target: { value: exampleReason },
		});
		fireEvent.click(screen.getByRole("button", { name: "Save explanation" }));
		...
		await waitFor(() =>
			expect(
				screen.queryByText(/Coaching is temporarily unavailable/),
			).toBeNull(),
		);
