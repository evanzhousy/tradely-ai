...
export function numberQuestion(
	...
	return {
		...
		prompt: t(en, zh),
		choices: [],
		input: { kind: "number", unit: t(unit, unitZh) },
		accepted: [String(answer)],
		tolerance,
		explanation: t(why, whyZh),
	};
}
export function choose(
	...
	return {
		id,
		prompt: t(en, zh),
		choices: choices.map(([id, en, zh]) => ({ id, label: t(en, zh) })),
		accepted: [accepted],
		explanation: t(why, whyZh),
	};
}
export function write(
	...
	return {
		...
		prompt: t(en, zh),
		choices: [],
		input: { kind: "text", minLength, maxLength: 1800 },
		accepted: [],
		explanation: t(exemplar, exemplarZh),
	};
}
...
export const quotes = {
	title: "OIC · Bid and ask",
	href: "https://www.optionseducation.org/news/understanding-the-bid-and-ask-prices-for-options",
};
export const oi = {
	title: "OIC · Open interest",
	href: "https://www.optionseducation.org/news/open-interest-why-it-matters",
};
export const greeks = {
	title: "OIC · Understanding options Greeks",
	href: "https://www.optionseducation.org/advancedconcepts/understanding-options-greeks",
};

export function unitScenarios(
	unit: TeachingUnit,
	purpose: "practice" | "evaluation" = "practice",
): LearningScenario[] {
	return (purpose === "practice" ? [1, 2] : [3]).map((variant) => {
		const build = (v: number, independent: boolean): ScenarioStep => {
			...
			return {
				...data,
				execution: undefined,
				id: independent ? "independent" : "guided",
				kind: independent ? "independent" : "guided",
				title: t(
					independent
						? "Apply the concept to new evidence"
						: "Work through a contrasting case",
					independent ? "将概念应用于新证据" : "练习对比案例",
				),
				facts: data.facts ?? [],
				evidence: [],
				requiredEvidence: [],
				...
				questions: data.questions.map((question, index) => {
					...
					return {
						...question,
						choices: [
							...question.choices.slice(shift),
							...question.choices.slice(0, shift),
						],
					};
				}),
			};
