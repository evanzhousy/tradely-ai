...
This runtime asset is exported from the editable `Tradely_Exchange_Human_Night_v3` Blender scene, based on the supplied exchange photograph and the generated documentary reference views. It retains seven circular trading posts, 380 market display quads, 24 tower display faces, architecture, glazing, overhead equipment, and charcoal carpet flooring. Proportions are photographic approximations rather than a measured reconstruction.
...
- Carpet maps: original seamless low-pile charcoal texture, generated by `scripts/trading-hall/apply_carpet_floor.py`; color, normal, and roughness maps with a half-metre repeat. The former wood-plank joints are hidden in Blender and excluded from the runtime model.
- Fascia texture maps: [Poly Haven natural walnut veneer](https://polyhaven.com/a/natural_walnut_veneer), CC0. Trading-post wood retains its own calibrated tint, scale, normal, and roughness settings.
